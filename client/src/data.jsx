...
async function apiFetch(path, options = {}) {
  const opts = {
    ...
    headers: {
      ...
      // Sunucunun kendi ürettiği metinler (dinamik hata mesajları, süre
      // etiketleri) arayüz dilini izlesin. İndirme bağlantıları başlık
      // taşıyamadığı için orada ?lang kullanılıyor.
      'X-Stoa-Lang': uiLang(),
      ...(options.headers || {}),
    },
  };
  ...
  if (!res.ok) {
    ...
    //
    // Çeviri burada, tek noktada yapılıyor. Sunucu hata metinleri Türkçe ve
    // yüzden fazla çağrı noktası bunları doğrudan err.message olarak
    // gösteriyor; her birini tek tek çevirmek yerine kod burada sözlükten
    // geçiriliyor. Sözlükte karşılığı yoksa sunucunun gönderdiği metne,
    // o da yoksa koda düşüyor.
    const kod = data.error || null;
    const cevrilmis = kod && window.t?.(kod) !== kod ? window.t?.(kod) : null;
    const err = new Error(cevrilmis || data.message || kod || `HTTP ${res.status}`);
    err.code = kod;
...
window.APP_I18N = {
  tr: {
    ...
    dash_dist_title:'Pano dağılımı', dash_dist_sub:'Kartların kolonlara göre şu anki dağılımı',
    ...
    board_no_done_col:'Bu panoda "tamamlandı" olarak işaretli kolon yok. Gecikme ve süre rakamları bitmiş işleri de sayıyor. Kolon başlığındaki menüden bitiş kolonunu işaretleyebilirsin.',
    ...
    rep_act_mcp_created:'Kart açıldı — MCP', rep_act_mcp_updated:'Kart düzenlendi — MCP',
    rep_act_mcp_moved:'Kart taşındı — MCP',
    rep_act_mcp_comment:'Yorum yazıldı — MCP',
    rep_act_mcp_deleted:'Kart çöpe atıldı — MCP',
    rep_act_mcp_restored:'Kart geri alındı — MCP',
    rep_act_mcp_sub_added:'Alt görev eklendi — MCP',
    rep_act_mcp_sub_updated:'Alt görev düzenlendi — MCP',
    rep_act_mcp_sub_deleted:'Alt görev silindi — MCP',
    rep_act_mcp_ws_switched:'Aktif alan değişti — MCP',
    ...
    err_assignee_not_member:'Atanan kişi bu çalışma alanının üyesi değil',
    err_workspace_forbidden:'Bu çalışma alanına erişiminiz yok',
    err_member_not_found:'Üye bulunamadı',
    err_member_not_in_workspace:'Bu çalışma alanında böyle bir üye yok',
    ...
    err_mcp_token_invalid:'MCP anahtarı geçersiz.',
    err_mcp_user_unknown:'MCP anahtarının kullanıcısı bulunamadı.',
    err_mcp_method_not_allowed:'MCP ucu yalnızca POST kabul eder.',
    err_mcp_rate_limited:'Çok fazla MCP isteği. Biraz bekleyin.',
    err_mcp_no_workspace:'Aktif çalışma alanı yok.',
    err_mcp_workspace_mismatch:'İstenen çalışma alanı aktif alan değil; hiçbir şey yazılmadı.',
    err_mcp_column_not_found:'Böyle bir kolon yok.',
    err_mcp_nothing_to_update:'Değiştirilecek alan verilmedi.',
    err_mcp_assignee_conflict:'Aynı kişi hem eklenip hem çıkarılamaz.',
    err_mcp_label_not_found:'Bu projede öyle bir etiket yok.',
    err_mcp_label_conflict:'Aynı etiket hem eklenip hem çıkarılamaz.',
    err_mcp_subtask_not_found:'Bu görevde öyle bir alt görev yok.',
    ...
    // Sunucu hata kodları (routes/*). Kod adı sözlük anahtarıyla aynı,
    // böylece apiFetch gelen kodu doğrudan çevirebiliyor.
err_action_forbidden:'Bu işlem için yetkiniz yok',
    err_add_member_forbidden:'Üye ekleme yetkiniz yok',
    err_allowed_next_not_array:'allowed_next bir dizi olmalı',
    err_attachment_not_found:'Ek bulunamadı',
    err_cannot_change_own_role:'Kendi rolünüzü değiştiremezsiniz',
    err_cannot_message_self:'Kendinize mesaj gönderemezsiniz',
    err_cannot_remove_channel_owner:'Kanal sahibini çıkaramazsınız',
    err_cannot_remove_self:'Kendinizi çıkaramazsınız',
    err_cannot_transfer_self:'Kendinize sahiplik aktaramazsınız',
    err_channel_delete_forbidden:'Kanalı sadece kanal sahibi veya yönetici silebilir',
    err_channel_forbidden:'Bu kanala erişim yetkiniz yok',
    err_channel_name_empty:'Kanal adı boş olamaz',
    err_channel_name_invalid:'Geçerli bir kanal adı girin',
    err_channel_not_found:'Kanal bulunamadı',
    err_channel_send_forbidden:'Bu kanala mesaj gönderme yetkiniz yok',
    err_channel_type_owner_only:'Kanal tipini sadece sahip değiştirebilir',
    err_column_not_found:'Kolon bulunamadı',
    err_comment_not_found:'Yorum bulunamadı',
    err_confirm_email_mismatch:'Hesabı silmek için e-posta adresinizi doğru yazın',
    err_create_channel_forbidden:'Kanal oluşturma yetkin yok. Workspace sahibiyle iletişime geç.',
    err_create_project_forbidden:'Proje oluşturma yetkiniz yok',
    err_create_task_forbidden:'Görev oluşturma yetkiniz yok',
    err_default_channel_delete:'Varsayılan kanal silinemez',
    err_default_channel_leave:'Varsayılan kanaldan ayrılamazsınız',
    err_default_channel_remove:'Varsayılan kanaldan üye çıkarılamaz',
    ...
    err_empty_trash_forbidden:'Çöp kutusunu boşaltma yetkiniz yok',
    err_file_name_empty:'Dosya adı boş',
    err_file_not_found:'Dosya bulunamadı',
    err_file_too_large:'Dosya 20 MB sınırını aşıyor',
    err_images_only:'Sadece resim dosyaları yüklenebilir',
    err_invalid_channel_type:'Geçersiz kanal tipi',
    err_invalid_file:'Geçersiz dosya',
    err_invalid_role:'Geçersiz rol',
    err_label_not_found:'Etiket bulunamadı',
    err_last_default_role:'Son varsayılan rol silinemez. Önce başka bir rol oluşturun.',
    err_manage_members_forbidden:'Üye yönetme yetkiniz yok',
    err_message_empty:'Mesaj boş olamaz',
    err_message_not_found:'Mesaj bulunamadı',
    err_no_active_workspace:'Aktif çalışma alanı yok',
    err_no_default_role:'Varsayılan rol bulunamadı. Lütfen bir varsayılan rol tanımlayın.',
    err_no_file_selected:'Dosya seçilmedi',
    err_not_channel_member:'Bu kullanıcı kanal üyesi değil',
    err_not_workspace_member:'Bu çalışma alanına üye değilsiniz',
    err_note_edit_forbidden:'Bu notu düzenleme yetkiniz yok',
    err_note_forbidden:'Bu nota erişiminiz yok',
    err_note_not_found:'Not bulunamadı',
    err_notification_not_found:'Bildirim bulunamadı',
    err_owner_remove:'Sahip takımdan çıkarılamaz',
    err_owner_required:'Bu işlem için sahip yetkisi gereklidir',
    err_owner_role_change:'Sahip rolü değiştirilemez',
    err_password_too_short:'Parola en az 8 karakter olmalıdır',
    err_project_name_required:'Proje adı zorunludur',
    err_remove_member_forbidden:'Üye çıkarma yetkiniz yok',
    err_request_already_handled:'Bu istek zaten işlendi',
    err_request_not_found:'İstek bulunamadı',
    err_role_assign_owner_only:'Rol atamayı sadece kanal sahibi yapabilir',
    err_role_name_required:'Rol adı zorunludur',
    err_role_not_found:'Rol bulunamadı',
    err_select_member:'En az bir üye seçmelisiniz',
    err_subtask_not_found:'Alt görev bulunamadı',
    err_target_member_missing:'Hedef üye belirtilmedi',
    err_task_wrong_workspace:'Bu görev bu çalışma alanına ait değil',
    err_title_required:'Başlık zorunludur',
    err_unauthorized:'Yetkisiz işlem',
    err_user_not_found:'Kullanıcı bulunamadı',
    err_user_not_in_team:'Bu kullanıcı aktif takımınızda değil',
    err_user_not_workspace_member:'Bu kullanıcı çalışma alanının üyesi değil',
    err_workspace_name_required:'Çalışma alanı adı zorunludur',
    err_workspace_not_found:'Workspace bulunamadı',
    // Sunucu hata kodları (routes/*). Kod adı sözlük anahtarıyla aynı,
    // böylece apiFetch gelen kodu doğrudan çevirebiliyor.
err_channel_name_taken:'Bu isimde bir kanal zaten var',
    err_column_ids_required:'column_ids array gerekli',
    err_comment_text_required:'Yorum metni zorunludur',
    err_delete_column_forbidden:'Kolon silme yetkiniz yok',
    err_forbidden:'Yetkiniz yok',
    err_note_delete_forbidden:'Bu notu silme yetkiniz yok',
    err_notification_text_required:'Bildirim metni gerekli',
    err_permission_required:'Yetki gerekli',
    err_slug_name_required:'slug ve name_en zorunludur',
    err_task_id_required:'task_id gerekli',
    // 3 Eylül 2026 dil turu — kodda çağrılıp sözlüğe hiç eklenmemiş
    // anahtarlar. Eksik oldukları için İngilizce arayüzde Türkçe
    // görünüyorlardı (window.t sessizce Türkçe'ye düşüyor).
    app_confirm:'Onayla',
    app_saving:'Kaydediliyor…',
    app_error_generic:'Bir hata oluştu',
    app_err_move_task:'Görev taşınamadı',
    app_err_open_task:'Görev açılamadı: ',
    board_tl_undated:'Tarihi belirsiz',
    chat_back:'Geri',
    chat_channel_icon:'Kanal simgesi',
    chat_section_perms:'Üye izinleri',
    chat_post_perm:'Mesaj gönderebilir',
    chat_perm_everyone:'Herkes',
    chat_perm_everyone_desc:'Tüm üyeler mesaj yazabilir.',
    chat_perm_admins:'Yöneticiler',
    chat_perm_admins_desc:'Sadece sahip ve yöneticiler.',
    chat_section_danger:'Tehlikeli bölge',
    chat_leave_title:'Kanaldan Ayrıl',
    chat_leave_confirm_body:'kanalından ayrılmak istediğinize emin misiniz?',
    chat_leave_confirm_hint:'Ayrıldıktan sonra tekrar davet edilmeniz gerekebilir.',
    chat_delete_channel_hint:'Bu işlem geri alınamaz. Kanaldaki tüm mesajlar silinecektir.',
    chat_transfer_hint:'Sen yönetici rolüne geçersin.',
    dd_select:'— Seç —',
    drawer_rename_hint:'Yeniden adlandırmak için çift tıkla',
    err_view_crashed:'Bir şeyler ters gitti',
    err_view_crashed_sub:'Bu bölüm yüklenirken bir hata oluştu. Yeniden deneyebilir ya da soldan başka bir bölüme geçebilirsiniz.',
    err_retry:'Yeniden dene',
    notes_delete_title:'Notu Sil',
    notes_delete_btn:'Sil',
    set_app_accent_custom:'Özel renk',
    tweak_accent_custom:'Özel renk',
    set_notif_work:'İş Bildirimleri',
    set_notif_tasks_desc:'Sana görev atandığında, bahsedildiğinde veya görevine yorum yapıldığında ekranda göster',
    // 3 Eylül 2026 dil turu, ikinci grup — kolon menüsü, takvim rehberi,
    // emoji kategorileri ve sonradan eklenip sözlüğe yazılmayan iki izin
    // etiketi (view_reports, manage_workspace).
    board_col_color:'Renk değiştir',
    board_col_rule:'Geçiş kuralı',
    board_col_rule_count:'kolon',
    board_col_rule_none:'kısıt yok',
    board_col_rule_help:'Bu kolondan hangi kolonlara geçilebilir? Hiçbiri seçili değilse kart her kolona taşınabilir.',
    board_col_rule_clear:'Kuralı kaldır',
    board_err_no_project:'Proje seçili değil.',
    board_err_col_create:'Kolon oluşturulamadı: ',
    cal_guide_pick_end:'Bitiş tarihini seçin — aynı güne tıklarsanız tek günlük görev oluşturulur.',
    cal_guide_pick_start:'+ Yeni görev oluşturmak için takvimde bir başlangıç tarihi seçin.',
    notes_emoji_frequent:'Sık Kullanılan',
    notes_emoji_faces:'Yüzler',
    notes_emoji_hands:'El & Vücut',
    notes_emoji_objects:'Nesneler',
    notes_emoji_food:'Yiyecek',
    notes_emoji_nature:'Doğa',
    notes_md_bold:'güçlü',
    notes_md_italic:'italik',
    notes_md_code:'kod',
    set_rol_pick:'— Rol seç —',
    set_rol_perm_reports:'Herkesin raporlarını gör',
    set_rol_perm_workspace:'Çalışma alanını yönet',
    set_transfer_pick_member:'Lütfen bir üye seçin.',
    set_transfer_failed:'Sahiplik aktarılamadı.',
    set_delete_email_required:'Devam etmek için hesabınızın e-posta adresini yazın.',
  ...
  en: {
    ...
    dash_dist_title:'Board distribution', dash_dist_sub:'Where the cards sit right now, by column',
    ...
    board_no_done_col:'No column on this board is marked as done. Overdue and duration figures still count finished work. You can mark the final column from the menu in its header.',
    ...
    rep_act_mcp_created:'Card created — MCP', rep_act_mcp_updated:'Card edited — MCP',
    rep_act_mcp_moved:'Card moved — MCP',
    rep_act_mcp_comment:'Comment added — MCP',
    rep_act_mcp_deleted:'Card trashed — MCP',
    rep_act_mcp_restored:'Card restored — MCP',
    rep_act_mcp_sub_added:'Subtask added — MCP',
    rep_act_mcp_sub_updated:'Subtask edited — MCP',
    rep_act_mcp_sub_deleted:'Subtask deleted — MCP',
    rep_act_mcp_ws_switched:'Active workspace changed — MCP',
    ...
    err_assignee_not_member:'Assignee is not a member of this workspace',
    err_workspace_forbidden:'You do not have access to this workspace',
    err_member_not_found:'Member not found',
    err_member_not_in_workspace:'No such member in this workspace',
    ...
    err_mcp_token_invalid:'MCP key is not valid.',
    err_mcp_user_unknown:'No user matches this MCP key.',
    err_mcp_method_not_allowed:'The MCP endpoint only accepts POST.',
    err_mcp_rate_limited:'Too many MCP requests. Please wait a moment.',
    err_mcp_no_workspace:'No active workspace.',
    err_mcp_workspace_mismatch:'The requested workspace is not the active one; nothing was written.',
    err_mcp_column_not_found:'No such column.',
    err_mcp_nothing_to_update:'No fields to change were given.',
    err_mcp_assignee_conflict:'The same person cannot be both added and removed.',
    err_mcp_label_not_found:'No such label in this project.',
    err_mcp_label_conflict:'The same label cannot be both added and removed.',
    err_mcp_subtask_not_found:'No such subtask on this task.',
    ...
    // Sunucu hata kodları (routes/*). Kod adı sözlük anahtarıyla aynı,
    // böylece apiFetch gelen kodu doğrudan çevirebiliyor.
err_action_forbidden:'You are not allowed to do this',
    err_add_member_forbidden:'You are not allowed to add members',
    err_allowed_next_not_array:'allowed_next must be an array',
    err_attachment_not_found:'Attachment not found',
    err_cannot_change_own_role:'You cannot change your own role',
    err_cannot_message_self:'You cannot send a message to yourself',
    err_cannot_remove_channel_owner:'You cannot remove the channel owner',
    err_cannot_remove_self:'You cannot remove yourself',
    err_cannot_transfer_self:'You cannot transfer ownership to yourself',
    err_channel_delete_forbidden:'Only the channel owner or an admin can delete the channel',
    err_channel_forbidden:'You do not have access to this channel',
    err_channel_name_empty:'Channel name cannot be empty',
    err_channel_name_invalid:'Enter a valid channel name',
    err_channel_not_found:'Channel not found',
    err_channel_send_forbidden:'You are not allowed to post in this channel',
    err_channel_type_owner_only:'Only the owner can change the channel type',
    err_column_not_found:'Column not found',
    err_comment_not_found:'Comment not found',
    err_confirm_email_mismatch:'Type your email address correctly to delete the account',
    err_create_channel_forbidden:'You are not allowed to create channels. Contact the workspace owner.',
    err_create_project_forbidden:'You are not allowed to create projects',
    err_create_task_forbidden:'You are not allowed to create tasks',
    err_default_channel_delete:'The default channel cannot be deleted',
    err_default_channel_leave:'You cannot leave the default channel',
    err_default_channel_remove:'Members cannot be removed from the default channel',
    ...
    err_empty_trash_forbidden:'You are not allowed to empty the trash',
    err_file_name_empty:'File name is empty',
    err_file_not_found:'File not found',
    err_file_too_large:'File exceeds the 20 MB limit',
    err_images_only:'Only image files can be uploaded',
    err_invalid_channel_type:'Invalid channel type',
    err_invalid_file:'Invalid file',
    err_invalid_role:'Invalid role',
    err_label_not_found:'Label not found',
    err_last_default_role:'The last default role cannot be deleted. Create another role first.',
    err_manage_members_forbidden:'You are not allowed to manage members',
    err_message_empty:'Message cannot be empty',
    err_message_not_found:'Message not found',
    err_no_active_workspace:'No active workspace',
    err_no_default_role:'No default role found. Please define a default role.',
    err_no_file_selected:'No file selected',
    err_not_channel_member:'This user is not a member of the channel',
    err_not_workspace_member:'You are not a member of this workspace',
    err_note_edit_forbidden:'You are not allowed to edit this note',
    err_note_forbidden:'You do not have access to this note',
    err_note_not_found:'Note not found',
    err_notification_not_found:'Notification not found',
    err_owner_remove:'The owner cannot be removed from the team',
    err_owner_required:'This action requires owner permission',
    err_owner_role_change:"The owner's role cannot be changed",
    err_password_too_short:'Password must be at least 8 characters',
    err_project_name_required:'Project name is required',
    err_remove_member_forbidden:'You are not allowed to remove members',
    err_request_already_handled:'This request has already been handled',
    err_request_not_found:'Request not found',
    err_role_assign_owner_only:'Only the channel owner can assign roles',
    err_role_name_required:'Role name is required',
    err_role_not_found:'Role not found',
    err_select_member:'You must select at least one member',
    err_subtask_not_found:'Subtask not found',
    err_target_member_missing:'No target member specified',
    err_task_wrong_workspace:'This task does not belong to this workspace',
    err_title_required:'Title is required',
    err_unauthorized:'Unauthorised action',
    err_user_not_found:'User not found',
    err_user_not_in_team:'This user is not in your active team',
    err_user_not_workspace_member:'This user is not a member of the workspace',
    err_workspace_name_required:'Workspace name is required',
    err_workspace_not_found:'Workspace not found',
    // Sunucu hata kodları (routes/*). Kod adı sözlük anahtarıyla aynı,
    // böylece apiFetch gelen kodu doğrudan çevirebiliyor.
err_channel_name_taken:'A channel with this name already exists',
    err_column_ids_required:'column_ids array is required',
    err_comment_text_required:'Comment text is required',
    err_delete_column_forbidden:'You are not allowed to delete columns',
    err_forbidden:'Permission denied',
    err_note_delete_forbidden:'You are not allowed to delete this note',
    err_notification_text_required:'Notification text is required',
    err_permission_required:'Permission required',
    err_slug_name_required:'slug and name_en are required',
    err_task_id_required:'task_id is required',
    // 3 Eylül 2026 dil turu — kodda çağrılıp sözlüğe hiç eklenmemiş
    // anahtarlar. Eksik oldukları için İngilizce arayüzde Türkçe
    // görünüyorlardı (window.t sessizce Türkçe'ye düşüyor).
    app_confirm:'Confirm',
    app_saving:'Saving…',
    app_error_generic:'Something went wrong',
    app_err_move_task:'Could not move the task',
    app_err_open_task:'Could not open the task: ',
    board_tl_undated:'No date',
    chat_back:'Back',
    chat_channel_icon:'Channel icon',
    chat_section_perms:'Member permissions',
    chat_post_perm:'Can post messages',
    chat_perm_everyone:'Everyone',
    chat_perm_everyone_desc:'All members can post messages.',
    chat_perm_admins:'Admins',
    chat_perm_admins_desc:'Owner and admins only.',
    chat_section_danger:'Danger zone',
    chat_leave_title:'Leave Channel',
    chat_leave_confirm_body:'channel — are you sure you want to leave?',
    chat_leave_confirm_hint:'You may need to be invited again after leaving.',
    chat_delete_channel_hint:'This cannot be undone. All messages in the channel will be deleted.',
    chat_transfer_hint:'You will be moved to the admin role.',
    dd_select:'— Select —',
    drawer_rename_hint:'Double-click to rename',
    err_view_crashed:'Something went wrong',
    err_view_crashed_sub:'An error occurred while loading this section. You can retry or switch to another section from the left.',
    err_retry:'Retry',
    notes_delete_title:'Delete Note',
    notes_delete_btn:'Delete',
    set_app_accent_custom:'Custom color',
    tweak_accent_custom:'Custom color',
    set_notif_work:'Work Notifications',
    set_notif_tasks_desc:'Show on screen when a task is assigned to you, you are mentioned, or someone comments on your task',
    // 3 Eylül 2026 dil turu, ikinci grup — kolon menüsü, takvim rehberi,
    // emoji kategorileri ve sonradan eklenip sözlüğe yazılmayan iki izin
    // etiketi (view_reports, manage_workspace).
    board_col_color:'Change color',
    board_col_rule:'Transition rule',
    board_col_rule_count:'columns',
    board_col_rule_none:'no restriction',
    board_col_rule_help:'Which columns can a card move to from here? If none are selected, the card can move to any column.',
    board_col_rule_clear:'Clear rule',
    board_err_no_project:'No project selected.',
    board_err_col_create:'Could not create the column: ',
    cal_guide_pick_end:'Pick an end date — click the same day for a single-day task.',
    cal_guide_pick_start:'+ Pick a start date on the calendar to create a new task.',
    notes_emoji_frequent:'Frequently Used',
    notes_emoji_faces:'Faces',
    notes_emoji_hands:'Hands & Body',
    notes_emoji_objects:'Objects',
    notes_emoji_food:'Food',
    notes_emoji_nature:'Nature',
    notes_md_bold:'bold',
    notes_md_italic:'italic',
    notes_md_code:'code',
    set_rol_pick:'— Select role —',
    set_rol_perm_reports:'View everyone’s reports',
    set_rol_perm_workspace:'Manage workspace',
    set_transfer_pick_member:'Please select a member.',
    set_transfer_failed:'Could not transfer ownership.',
    set_delete_email_required:'Type your account email address to continue.',
    // 3 Eylül dil turu, üçüncü grup — `}metin<` kalıbıyla bulunanlar.
